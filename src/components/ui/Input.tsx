...
/**
 * Raw presentational input (unchanged behaviour)
 */
    ...
    return (
      ...
      >
        {labelText && (
          <label
            htmlFor={inputId}
            className=" justify-start  text-base font-medium  text-neutral-600 text-start"
          >
            {t(labelText)}
          </label>
        )}
...
type FormInputProps = Omit<TextInputProps, "defaultValue"> & {
  ...
  defaultValue?: any;
  showError?: boolean; // whether to render the error message (default true)
};

/**
 * FormInput:
 * - if `control` prop is provided => renders RHF Controller and shows error message from fieldState
 * - otherwise => renders plain Input (uncontrolled) and no RHF behaviour
 */
export const FormInput: React.FC<FormInputProps> = ({
  ...
  // If control provided, use Controller
  if (control) {
    return (
      <Controller
        name={name}
          ...
          return (
            <div className="w-full">
              <Input
                ...
                // spread RHF field props onto input (value & onChange)
                value={field.value ?? ""}
  ...
  // No control: render plain Input (no RHF)
  return (
